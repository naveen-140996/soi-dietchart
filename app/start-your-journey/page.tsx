...
const page = () => {
  ...
  return (
    <div>
        <Header />
        <div className="min-h-screen bg-gray-100 p-2 md:p-8 w-full  overflow-x-hidden">
        {/* Title */}
        ...
        >
          <h2 className="text-lg font-semibold mb-4">Day {currentDay}</h2>
          ...
          <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {history.map((item, index) => (
              <div key={index} className="bg-white p-4 rounded-xl shadow">
                ...
                <p className="text-sm">🍛 {item.lunch}</p>
                <p className="text-sm">🌙 {item.dinner}</p>
                <p className="text-sm font-medium mt-2">⚖️ {item.weight} kg</p>
              </div>
            ))}
          </div>
        </div>
